...
int enqueue(int data, char wisata[30], queue *myQueue){   // MASUKKAN NILAI KE QUEUE
int success;
...
void insertNode(node **head, node *pPre, node *pNew){   // MASUKKAN DATA KE NODE
    if (pPre == NULL){
	    pNew -> next = *head;
       *head = pNew;
    }
...
void deleteNode(node **head, node *pPre, node *pCur){   // HAPUS DATA KE NODE
	if(pPre == NULL)
		*head = pCur -> next;
	else
		pPre->next = pCur->next;
...
void tranverse(node *head){   // TAMPILKAN DATA-DATA
	node *pWalker;
	pWalker = head;
    system("cls");
	...
	while(pWalker != NULL){
		...
		if (pWalker->tanggal < 10){ // JIKA NILAI KURANG DARI 10
			printf("|0%d| ", pWalker -> tanggal);
			goto w;
		}

		printf("|%d| ", pWalker -> tanggal);w:
	...
	while(pWalker != NULL){
		...
		if (pWalker->tanggal-31 < 10){ // JIKA NILAI KURANG DARI 10
		...
		printf("|%d| ", pWalker -> tanggal-31); q://-1
		pWalker = pWalker -> next;
	...
	while(pWalker != NULL){
		...
		if (pWalker->tanggal-62 < 10){ // JIKA NILAI KURANG DARI 10
		...
		printf("|%d| ", pWalker -> tanggal-62); e://-1
		pWalker = pWalker -> next;
...
void hapusData(node **head, int tanggal){   // MENGHAPUS DATA
	node *pCur, *pPre;
	int pos;
...
void insert(node **root, node *newNode){  // MASUKKAN DATA KE NODE
	if(*root == NULL)
		*root = newNode;
...
void inorder(node *root, int *a, int *b, int *c){   // TAMPILKAN DATA
...
//=========================================================================================================================================================
		...
		if (p==1){   // PANEL PENGUNJUNG
			...
			else if (pil==2){   // LIHAT DENAH TEMPAT DUDUK
				system("cls");
				tampil();
				getch();
			}

			else if (pil>3 || pil<1){   // APABILA PENGUNJUNG MEMBERI MASUKKAN SELAIN 1-3
...
//=========================================================================================================================================================
			...
			if (strcmp("admin", name) == 0 && strcmp("admin", pass) == 0){   // PROSES AUTENTIKASI
				...
				else if (pil==3){   // LIHAT tanggal KOSONG - SINGLE LINKED LIST
					tranverse(head);
					getch();
				}
